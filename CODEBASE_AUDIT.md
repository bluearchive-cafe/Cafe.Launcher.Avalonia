...
- High：0
- Medium：0
- Low：5 open（AUD-PERF-004、AUD-PERF-005 残留、AUD-SEC-001、AUD-SEC-002、AUD-ARCH-005）+ 5 accepted-risk（PERF-001、MAINT-002、SEC-004、SEC-006、ARCH-007）
...
2.（**实测后建议不做**）AUD-PERF-004 横幅位图备忘：处置 Refactor，但 2026-09-17 的实测把收益定在约 10 ms/次刷新的后台 CPU（低两到三个数量级于该刷新本身的网络开销），不抵所有权移交与失效规则的复杂度；系数与重开判据（10 张 1200×500 ≈ 100 ms 量级）见条目。
