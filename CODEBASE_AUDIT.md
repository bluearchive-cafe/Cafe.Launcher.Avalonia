...
- 工具证据：`dotnet list package --vulnerable --include-transitive`（无漏洞）；**单元测试套件本地实测（Debug，`715fee5`）：1704 通过 / 0 失败 / 2 可见跳过**；`git diff`/`git log --name-only` 热点与依赖零变更核对；`wc -l` 规模核对；多处 grep（证书覆盖、ConnectCallback、UID 日志面、静态字段、Assert.Skip 计数）。
