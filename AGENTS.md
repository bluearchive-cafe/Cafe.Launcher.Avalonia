...
- Do **not** use `actions/setup-dotnet`'s `cache: true`. Its key is `dotnet-cache-<runner.os>-<hashFiles(csproj, lock files, Directory.Packages.props)>`, so any byte change in those files writes a **new** 600–800 MB cache entry — old entries are neither reused nor deleted, and the 10 GB per-repository cap gets pinned. The workflows instead pair `actions/cache/restore` with `actions/cache/save` under `key: nuget-global-packages-${{ runner.os }}-${{ hashFiles('Directory.Packages.props') }}` and a `restore-keys` prefix, so the key moves only when the central version manifest moves and a version bump reuses the previous generation before writing one new entry. They are two steps rather than one `actions/cache` call because that action's save is a `post-if: success()` step: while the test suite is red, a single-step cache never gets written. `actions/cache/save` therefore carries `if: always()` and takes its key from `steps.nuget-cache.outputs.cache-primary-key`; an existing key makes the save a no-op.
...
- Launcher **self-update** is Windows-only and reverses the earlier "never execute our own binary" decision ([ADR-037](docs/design/adr/ADR-037-Windows自更新走独立helper且以SHA256SUMS为信任锚.md)): `Services/Update/LauncherSelfUpdateService` selects the release asset for this host (installer build → `_setup.exe`, portable build → `_win-x64.zip`; detected by the `.cafe-launcher-install` marker), downloads it beside the release's `SHA256SUMS`, and verifies the SHA-256 before anything is applied. Applying is a separate process: `src/Cafe.Launcher.Updater` (a self-contained single-file helper copied to `%TEMP%`) waits for the launcher to exit, then runs the installer silently via `runas` or swaps the portable directory, and relaunches. The staged package is a necessary input to the selection — a release without `SHA256SUMS` or the expected asset degrades to the browser hand-off, never an unverified run. Non-Windows hosts keep the browser hand-off.
...
- A folded technical appendix is allowed at the end of the file: one `<details>` block after the user-facing notes, describing implementation-level changes (mechanisms, boundaries, engineering work). Lead it with a heading and one plain sentence *outside* the tags, because the in-app release-notes viewer ignores raw HTML blocks — text inside an HTML block silently disappears there, so leaving the blank line after the `<summary>` line is what keeps the entries readable in the app. Its contents are exempt from the vocabulary rule above; `ReleaseChangelogContractTests` scans only the text outside the fold, and guards that the appendix stays single, comes last, and cannot swallow the notes.
...
`CODEBASE_AUDIT.md` is the single source of truth for repository audit findings, accepted risks, and priorities. Keep it as a focused current-state document: verify a candidate before adding it, update an existing stable ID when the same root cause persists, move accepted items to its accepted-risk table, and remove resolved items from the current view. Git history is the audit history; do not recreate `.repository-audit/`, dated audit reports, or parallel candidate ledgers.
