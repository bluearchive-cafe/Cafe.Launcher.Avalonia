...
- `SettingsGeneralSection.axaml`, `SettingsGameSection.axaml`, `SettingsDownloadNetworkSection.axaml`, `SettingsAppearanceSection.axaml`, `SettingsAboutSection.axaml` — the five settings sections; all share the owning `MainWindowViewModel` data context and use the shared `SettingRow` control for consistent row layout
...
| `GameInstallationPath` | Computes the default game path and normalizes paths to `YostarGames\BlueArchive_JP`. Default path is the launcher's parent directory (`AppContext.BaseDirectory/..`) + `YostarGames\BlueArchive_JP`, matching the official launcher's `dirname(exe)/..` default (`request-default-download-path`, `index.js:742-743`) — game sits sibling to the launcher folder, safe from self-update overwrites |
...
1. Add the key to all 4 language dictionaries (`en`, `zh-Hans`, `zh-Hant`, `ja`)
