...
For UI, theme, or design-token work, read [docs/design/design-system-agent-guide.md](docs/design/design-system-agent-guide.md) (the agent-facing governance layer: numeric accessibility baselines, M3 container/elevation pairing, repository anchors) together with [docs/design/design-system-spec.md](docs/design/design-system-spec.md) and the accepted decisions under `docs/design/adr/`. The guide is secondary guidance: where it disagrees with an accepted ADR or the spec, the ADR and spec win.
