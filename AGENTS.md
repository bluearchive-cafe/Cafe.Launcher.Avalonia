...
A version bump also updates two documents that duplicate those versions, and Dependabot only writes the first. Update the §12 toolchain table in `PROJECT_CONVENTIONS.md` by hand — `InstallerContractTests.ProjectConventionsToolchainTable_MatchesDeclaredPackageVersions` fails while any row disagrees with `Directory.Packages.props`, which is how a bump lands a red CI instead of a mergeable PR. Then regenerate `THIRD-PARTY-NOTICES.md` with `.\scripts\New-ThirdPartyNotices.ps1`; no test guards its versions, so a stale entry drifts silently until the next audit. That script reads the resolved dependency graph, so it needs a prior `dotnet restore` in the same working tree.
