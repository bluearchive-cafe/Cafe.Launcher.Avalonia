...
public interface ISettingsEditor : INotifyPropertyChanged
{
    ...
    void MarkSaved(LauncherSettings settings);
...
public sealed class SettingsEditor : ISettingsEditor
{
    ...
    public void MarkSaved(LauncherSettings settings)
    {
        snapshot = settings.DeepClone();
        UpdateDirtyState();
    }
    ...
    {
        var newIsDirty = !LauncherSettingsContract.Matches(current, snapshot);
