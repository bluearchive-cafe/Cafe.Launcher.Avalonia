...
- 新增族（`App.axaml` 按注释分区）：`Launcher.Elevation.*`（0–3 档阴影值：颜色/偏移/模糊；把 `MainWindow.Styles.axaml` 2 处硬编码 BoxShadow 迁为引用）、`Launcher.StateLayer.*`（8%/12%/12%/24%——pressed 原为 16%，随审计-0906 勘误为 12%，见 ADR-004 修订）、`Launcher.Color.Overlay.Scrim*`（3 档，替代/并列现有 overlay 用途）、`Launcher.Spacing.Thickness.*` 补齐全档、`Launcher.Typography.*` 角色键（P1 仅为别名/新增，消费方 P2 迁移）。
