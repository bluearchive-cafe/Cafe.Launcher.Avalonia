...
6. **前缀与 `maps` 归属暂未接入**：它们需要调用方提供 prefix 与安装目录上下文，本片未改 `IGameProcessTracker` 契约；标记已覆盖计划 P0-B 的三条完成标准（跨启动器重启、宿主退出、不误报）。
...
- 「前缀 / `maps` 归属」仍是设计稿 §3 的强信号，待接线（需要扩展 `IGameProcessTracker` 查询上下文）。
