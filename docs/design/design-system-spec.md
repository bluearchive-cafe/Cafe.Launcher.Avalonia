...
- 键盘：全表面 Tab 序、可见焦点环（`Button:focus-visible` 等既有规则随重命名迁移）、覆盖层焦点陷阱（`OverlayFocusBehavior` 保留）。`wizard-option`（ADR-017）与设置导航/更新文件列表同样接入 token 焦点环（审计-0828 补全）；`CheckBox` 的焦点环按**可见盒**分派：`:empty`（当前全部 CheckBox，可见标签一律在控件外——`SettingRow` 的 Title/Description 或同级文本行）铺在勾选框字形 `Border#NormalRectangle` 上；`:not(:empty)`（将来若给控件本身挂原生标签）铺在控件盒 `Border#PART_Border` 上，此时盒宽恰为字形+间距+标签。之所以不能沿用默认装饰器：Fluent 的布局盒因 `MinHeight` 32 加上空标签预留的 8px 间距比字形大（实测 28×32 vs 20×20），装饰器铺满布局盒时会相对勾选框偏右下，故 `FocusAdorner` 置空并改铺 token 环；两条分支各由一条 headless 契约锁定。**若日后真给 CheckBox 挂原生标签**，还须把标签字色接入 token——Fluent 的 `CheckBoxForeground*` 既不随 `Launcher.Text.*` 也不随中性色策略，不接就会重演组件库默认色盖过语义层；`FocusRing` 为运行时 scheme 的不透明 `Primary`，由 `MaterialSchemeGeneratorTests` 对对话框、卡片、内容行、字段、站点按钮与 Toast 表面自动校验 ≥3:1，走查清单继续覆盖真实渲染、边缘与高对比模式。
