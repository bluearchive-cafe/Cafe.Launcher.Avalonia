...
- 动效家族统一为即时反馈、内容切换、空间连续、临时表面与重要完成。常规控件不缩放、不弹跳；模态表面只移动外壳一次，内部内容不重复移动；自动轮播只淡化。向导步骤切换属空间连续家族的方向变体：顺序两段换页（先退出加速淡出、后进入减速滑入 ±14px；入场前以起势帧消化布局、收尾留缓冲再结算，换页墙钟约 290ms），由后置代码编排并可中断（ADR-017）。
