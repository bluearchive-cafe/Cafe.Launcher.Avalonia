...
| D 结构收敛 | 16 落地／1 判定不做 | `d2d4bc6` `8aab531` `588d80c` `bf06ec0`…`5253311`、`01ddc1a`（D7）、`b57bd06`（D17）、`d0f4496`（D15）；`D13` 2026-09-19 判定不做 | `D1`（即 `DEF-1`）、`D14`、`D3` ＋ A 组 10 项 ＋ B 组 `D7`（取命名常量方案，未动文案与 golden）、`D17`、`D15`（= `AUD-ARCH-005`/评审 c07，Attach 记录式拆卸）；`D13` 用户裁定本轮不做 |
...
> **状态：已落地 16／17＋1 判定不做——A 组 10 项（零外溢）全部完成（`bf06ec0`…`5253311`，逐项独立提交 ＋ 逐项门禁），
> 加上此前的 `D1` → `d2d4bc6`、`D14` → `8aab531`、`D3` → `588d80c`、`D16` → `5253311`；
> B 组三项 2026-09-19 落地：`D7` → `01ddc1a`（取命名常量方案，未动文案与 golden）、
> `D17` → `b57bd06`（迁 ObservableObject，属性名契约不变）、`D15` → `d0f4496`（按 §5 决策一取 Attach 记录式拆卸，= `AUD-ARCH-005` 结案）。**
> `D13` 2026-09-19 用户裁定本轮不做（理由：① 的 `UiStyleContractTests` 断言改写与 golden 风险需单独一轮处理；② 含代码后置动画不能纯抽）。
>
...
2026-09-19 收口：`D15`（按 §5 决策一）、`D7`、`D17` 已落地；`D13` 判定不做。阶段 D 无剩余项。
...
### 决策一：`ShellLifecycle` 的订阅收敛形态（`D15` / 评审候选 07）—— **已裁定并落地：Attach 记录式拆卸（`d0f4496`）**
