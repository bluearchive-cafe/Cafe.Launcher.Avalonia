...
- 动效（ADR-016/017）：步骤切换 = 顺序换页——空间档 333ms 对半分：旧内容 166.5ms 加速淡出（禁命中）→ 中点换内容并复位滚动 → 起势帧渲染入场 pose（消化首次布局）→ 新内容 166.5ms 减速淡入 + ±14px 方向滑入（透明度与位移同批 `DoubleTransition`，收尾留缓冲再结算）；快速连点最新状态生效；外壳遮罩 Fast 淡入、表面单次进出场；降动效瞬切换面并定格。
