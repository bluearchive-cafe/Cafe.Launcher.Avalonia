...
- 日志文件操作区使用与游戏管理行相同的水平 `StackPanel`，通过 `LauncherSpacingSm` 保持按钮间距。
...
- 在默认 `1300×754` 和最小 `1024×640` 窗口尺寸下，日志级别选择框和三个日志按钮保持同行、正尺寸、无重叠且位于滚动内容区内。
- 保持设置对话框现有固定最大尺寸，分类切换时不改变窗口高度。
- 页面剩余空白是低密度分类在稳定对话框中的自然结果，不通过无关内容、卡片或装饰元素填充。
- 若本地化文本增长，按钮保留内容所需宽度；布局测试负责防止操作区侵入标题说明区域。
